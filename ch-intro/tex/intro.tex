...
Defined as a set of items and the connections between them \cite{The
  Structure and Functino of Complex Networks}, networks provide a
...
\cite{Evolutionary conservation of motif constituents in the yeast
  protein interaction network}. Economist have constructed network
models of trade and determined that highly connected networks are more
sensititve to price variations \cite{Spatial price dynamics: From
  complex network perspective}. Applications can even be found in
...
interest \cite{An SIS model with infective medium on complex
  networks}. Ideally an analytical equation governing the evolution of
the quantity of interest would be readily derived from a description
of the fine-scale dynamics; however, as model complexity increases in
an effort to better match real-world conditions, deriving such
closed-form expressions becomes increasingly difficult. In such cases,
equation free modeling may enable the desired macroscopic analysis
despite the lack of explicit equations. This approach has been
successfuly applied to simple models of network evolution \cite{bold},
and we extend this work to hypergraph models in which multiple connections
are permitted between nodes.
...
network structure are clear, see for instance \cite{ER; Chung Lu;
  Barabasi; Watts}. In more complicated models the picture is muddier,
and it would helpful to have an automated method of identifying
whether such a simpler description is present. Towards this end, we
extend the dimensionality-reduction technique Diffusion Maps to
parameterize collections of hypergraphs. This requires a suitable
measure of the similarity between two networks, a problem which is
